...
class TestClass(object):
    ...
    #
    # @pytest.mark.skip(reason="test_bm25_new: test for the old class structure")
    ...
    def test_document_questions(self):
        ...
        query = 'All over! Buy it its worth every penny.'
        doc = Document(data, query, CONST.COL_QUESTION)
        dic = doc.get_scored_docs()
        # print dic
        assert type(dic) == type(data)

    @pytest.mark.skip(reason="test_documents_reviews: Need better review data")
    def test_documents_reviews(self):
        review_data = [
            {'reviewerID': 'A3G6XNM240RMWA', 'asin': '7806397051', 'summary': 'great quality', 'reviewerName': 'Karen',
             'helpful': [0, 4], 'unixReviewTime': 1378425600, 'reviewText': u'Great', 'overall': 4.0,
             'id': '5a4749acf1cca0eadf9574fe', 'reviewTime': '09 6, 2013'},
            {'reviewerID': 'A3G6XNM240RMWAeee', 'asin': '7806397051', 'summary': 'great quality',
             'reviewerName': 'Karen',
             'helpful': [0, 4], 'unixReviewTime': 1378425600, 'reviewText': u'Very Bad', 'overall': 4.0,
             'id': '5a4749acf1cca0eadf9574fe', 'reviewTime': '09 6, 2013'},
            {'reviewerID': 'A3G6XNM240RMWA', 'asin': '7806397051', 'summary': 'great quality', 'reviewerName': 'Karen',
             'helpful': [0, 4], 'unixReviewTime': 1378425600, 'reviewText': u'Yes', 'overall': 4.0,
             'id': '5a4749acf1cca0eadf9574fe', 'reviewTime': '09 6, 2013'}]
        query = 'All over! Buy it its worth every penny.'
        doc = Document(review_data, query, CONST.COL_REVIEW)
        dic = doc.get_scored_docs()
        assert type(dic) == type(review_data)
